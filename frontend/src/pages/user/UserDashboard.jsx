...
const UserDashboard = () => {
    ...
    const fetchComplaints = async () => {
        try {
            ...
            const response = await fetch(`http://${window.location.hostname}:5000/api/complaints/user/${userId}`, {
                headers: {
    ...
    const markAsRead = async (messageId) => {
        try {
            const token = getToken();
            const response = await fetch(`http://${window.location.hostname}:5000/api/messages/${messageId}/read`, {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex space-x-4 mb-8">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg font-medium transition-all ${activeTab === 'complaints' ? 'bg-blue-600 text-white' : 'glass text-gray-500 hover:text-gray-900'}`}
                    >
                        Complaints
                    </button>
                    <button
                        onClick={() => setActiveTab('messages')}
                        className={`px-4 py-2 rounded-lg font-medium transition-all ${activeTab === 'messages' ? 'bg-blue-600 text-white' : 'glass text-gray-500 hover:text-gray-900'}`}
                    >
                        ...
                        <div className="glass rounded-xl p-6 mb-8">
                            ...
                            <div className="flex flex-wrap gap-4">
                                ...
                                <Link
                                    ...
                                    className="px-6 py-3 glass text-gray-700 hover:bg-gray-100 rounded-lg font-medium transition-all duration-300"
                                >
                                    View All Complaints
                                </Link>
                            </div>
                                    ...
                                    <table className="w-full">
                                        <thead>
                                            <tr className="border-b border-gray-200">
                                                ...
                                                <th className="text-left py-3 px-4 text-gray-700 font-medium text-sm">Priority</th>
                                                <th className="text-left py-3 px-4 text-gray-700 font-medium text-sm">Date</th>
                                        ...
                                        <tbody>
                                            {recentComplaints.map((complaint) => (
                                                ...
                                                >
                                                    <td className="py-4 px-4 text-gray-900 font-mono text-sm">{complaint.complaint_id}</td>
                                                    <td className="py-4 px-4 text-gray-600">{complaint.priority}</td>
                                                    <td className="py-4 px-4 text-gray-500 text-sm">{new Date(complaint.createdAt).toLocaleDateString()}</td>
                                                    <td className="py-4 px-4">
                                                        ...
                                                        {(complaint.status === 'Flagged' || complaint.status === 'Rejected') && (
                                                            <div className={`mt-2 text-xs p-2 rounded ${complaint.status === 'Rejected' ? 'text-red-700 bg-red-50' : 'text-yellow-700 bg-yellow-50'}`}>
                    ...
                    <div className="space-y-4">
                        {messages.length > 0 ? (
                                ...
                                <div key={m._id} className={`glass p-6 rounded-xl relative transition-all ${!m.read ? 'border-l-4 border-blue-500' : 'opacity-80'}`}>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <span className="text-xs text-gray-400 block mt-2">{new Date(m.createdAt).toLocaleString()}</span>
                                        </div>
                                        {!m.read && (
                                            <button
                            ...
                            <div className="text-center py-12 glass rounded-xl">
                                <p className="text-gray-500">No official messages yet.</p>
                            </div>
                        )}
                    </div>
                )}
