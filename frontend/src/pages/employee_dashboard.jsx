          ...
          <div className="space-y-6">
            ...
            {selectedTask ? (
              <div className="glass rounded-xl p-6">
                ...
                {(selectedTask.representative_images?.length > 0 || selectedTask.complaints?.some(c => c.image)) && (
                  <div className="glass rounded-xl p-6 mb-6">
                    <h4 className="text-gray-900 font-medium mb-3">📷 Citizen-Submitted Photo</h4>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {(selectedTask.representative_images?.length > 0
                        ? selectedTask.representative_images
                        : selectedTask.complaints.filter(c => c.image).map(c => c.image)
                      ).map((img, i) => (
                        <img
                          key={i}
                          src={img}
                          alt={`Complaint evidence ${i + 1}`}
                          className="w-full h-32 object-cover rounded-lg cursor-pointer hover:scale-105 transition-transform border border-gray-200"
